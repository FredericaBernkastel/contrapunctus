...
accompanying voice moves in the subject's rhythm — every voice that is sounding, which since
[§8.17](#817-silence-and-what-the-voice-count-really-costs) is no longer all of them in the exposition and is still
all of them after it — every episode is a strict sequence where
...
#### The cheapest rule, which is now what the generator does
...
no parameter attached to it. Free voices per block under it — `E` exposition, `L` link, `m` middle, `c` close, and
the table is generated by the same function the generator runs, so it cannot disagree with the music:
...
**The first block is one voice alone**, which is a case the search had never been given: `realise::fill` refuses a
problem with no free voice in it, and is right to, so a solo line is now answered by `fill_block` directly instead
of searched. A single voice needs no counterpoint.

The link is `2` rather than `1`, and that is not a rounding. `derive` hands the link's motive to the voice that is
*about* to enter, so that voice sounds there before it has stated anything — and the held voice is never silent,
since a block is a line placed in a voice. Counting the link as though its holder had entered was the first version
of this table, which is why the table is now read off `resting` rather than recomputed beside it.
...
nothing: the first blocks drop to zero, one and two free where the whole piece used to be two.
...
**It changes the music, and the mechanism for saying so already exists.** A settings file written before this
reproduces `Differs` rather than `Exact`, because voices that used to play through the exposition now rest. That is
...
6. **Selectivity**, which [§8.6](#86-realisation-and-the-first-notes) turned from a prediction into a number:
   ...
   **The free part is built and it is not enough.** `compose::resting` rests a voice until it has entered — no
   parameter, no choice, the derivation already says who enters when — and a fugue now opens with one voice, then
...
One further piece of somebody else's work is **in the tree** rather than pinned beside it, and the interface is
where it is used.
